...
function Createdcourse() {
    ...
    async function fetchData() {
      try {
        ...
        if (token) {
          ...
          const userResponse = await axios.get(`https://eduvance-backend.onrender.com/user/${decodedToken.userId}` , {
            headers: {
          ...
          const coursesResponse = await axios.get('https://eduvance-backend.onrender.com/created-courses', {
  ...
  const handleDeleteCourse = async (courseId) => {
    try {
      const token = localStorage.getItem('token');
      await axios.delete(`https://eduvance-backend.onrender.com/delete-course/${courseId}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
