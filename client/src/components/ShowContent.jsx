...
function ShowContent() {
  ...
  const fetchCourseData = useCallback(async () => {
    try {
      const courseContentResponse = await axios.get(`https://eduvance-backend.onrender.com/my-courses/${courseId}`);
      ...
      const completedCoursesResponse = await axios.get(`https://eduvance-backend.onrender.com/completed-lectures`, {
  ...
  const showPdf = (url) => {
    window.open(`https://eduvance-backend.onrender.com/files/${url}`, "_blank");
  };

  const handleOnPlay = async () => {
    try {
      const response = await axios.post(`https://eduvance-backend.onrender.com/add-course-progress`, {
          ...
          <div className='my-5'>
              {selectedLecture && (
                <>
                  ...
                  {/* <Link to={`/certificate/${courseId}/${courseName}`}></Link> */}
                  {selectedContent === 'certificate' && (
                    <>
                      <br />
                      <Link to={`/quiz-instruction/${courseId}/${courseName}`}>
                        <button type="button" className="btn btn-success">
                          Attempt Quiz to Get Your Certificate
                        </button>
                      </Link>
                    </> 
                  )}
