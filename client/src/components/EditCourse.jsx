...
function EditCourse() {
  ...
  const [courseData, setCourseData] = useState({
    ...
    const fetchCourse = async () => {
      try {
        const response = await axios.get(`https://eduvance-backend.onrender.com/edit-course/${courseId}`, {
          headers: {
            Authorization: `Bearer ${token}`
          }
        });
  ...
  const handleEditCourse = async () => {
    ...
    for (const lectureData of courseData.lectureIds) {
      ...
      if (lectureData._id) {
        try {
          const response = await axios.post(`https://eduvance-backend.onrender.com/update-lecture/${lectureData._id}`, lectureData, {
            headers: {
              'Authorization': `Bearer ${token}`
            }
          });
    ...
    try {
      ...
      await axios.put(`https://eduvance-backend.onrender.com/edit-course/${courseId}`, updatedCourseData, {
        headers: {
          Authorization: `Bearer ${token}`
        }
      });
